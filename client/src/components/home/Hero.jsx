...
const Hero = () => {
  ...
  return (
    <section className="relative block sm:flex w-full h-[82vh] bg-black">
      {/* Left Side Black Box */}
      <div
        className="absolute justify-center sm:justify-end z-10 left-0 bottom-0 sm:bottom-0 sm:left-[-8%] w-full sm:w-2/3 h-80 sm:h-full bg-black text-white px-4 sm:px-16 flex items-center justify-end 
        skew-y-[-10deg] sm:skew-y-0 sm:skew-x-[-10deg]" // Y-skew on mobile, X-skew on larger screens
      >
        <div className="absolute sm:px-4 transform -skew-y-[-10deg] sm:skew-y-0 sm:-skew-x-[-10deg] justify-items-center sm:justify-items-end">
          {/* First Paragraph Text */}
          <p className="md:text-lg text-[14px] mb-4  text-center sm:text-right">
            Hey! it's me, here
          </p>

          {/* Heading Text */}
          <h2 className="text-1xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-rose-500 animate-float  text-center sm:text-right">
            {currentText}
          </h2>

          {/* Second Paragraph Text */}
          <p className="md:text-1xl text-1xl mt-4 text-center sm:text-right">
            Coming soon
          </p>

          {/* Buttons */}
          <div className="mt-6 flex justify-right sm:justify-end gap-6 sm:gap-4">
            <button onClick={() => window.location.href = "/portfolio"} className="px-4 sm:px-6 py-2 sm:py-3 text-sm sm:text-md text-white-500 bg-transparent border-2 border-rose-500 hover:bg-rose-500 hover:text-black transition-all duration-300 relative overflow-hidden">
              Portfolio
            </button>
            <button onClick={() => window.location.href = "/contact"} className="px-4 sm:px-6 py-2 sm:py-3 text-sm sm:text-md text-white-500 bg-transparent border-2 border-rose-500 hover:bg-rose-500 hover:text-black transition-all duration-300 relative overflow-hidden">
              Inquiry
            </button>
          </div>
        </div>
      ...
      <div className="absolute w-full sm:w-1/2 right-0 h-[600px] sm:h-full z-5">
        <img
